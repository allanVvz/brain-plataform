...
| GJ2-009 | Clicking the Tree control could appear to do nothing or reopen only one focused node. | The control represented Tree by deleting the `mode` query parameter and preserved a stale `focus`. A first fix still depended on an App Router `replace()` that production could reconcile back to the old URL. | Tree and Graph are now explicit accessible links. Selecting either performs a deterministic page navigation with the requested mode and no stale focus; legacy/unknown mode values normalize to Tree. |
