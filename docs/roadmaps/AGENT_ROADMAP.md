...
| 12 | Submissão de templates WhatsApp à Meta (Entrega 2 de campanhas) | **implementado 2026-09-12 — antes só registrava referência a um template já aprovado manualmente no WhatsApp Manager; agora existe criar rascunho local → `submit` (cria na Meta) → `edit` (resubmete se já submetido, bloqueado para nome/idioma/provider) → `sync` sob demanda (sem webhook de status ainda, decisão deliberada). Ver `docs/architecture/BULK_CAMPAIGNS.md`, seção "Templates Meta — ciclo de vida", com o exemplo real do template de boas-vindas da VZ Lupas pronto para submissão.** | — |
