...
Este repositorio deve rodar sem dependencia de Supabase Cloud, Vercel ou Cloud Run. O banco fica no Docker e a API usa uma stack local compativel com Supabase:

- Postgres + pgvector em `localhost:54322`
- PostgREST via gateway local em `http://localhost:54321/rest/v1`
- Backend FastAPI em `http://localhost:8000`
- Dashboard Next.js em `http://localhost:3000`
- Storage de arquivos em filesystem local (`LOCAL_STORAGE_DIR`), servido pela API em `/knowledge/file`
...
Na primeira subida, o volume `brain_postgres_data` executa todos os arquivos em `supabase/migrations/*.sql` e cria o admin local:

```text
email: allan@brain.com
senha: 123456
role: admin
```
...
```powershell
Invoke-WebRequest -UseBasicParsing http://localhost:54321/health
Invoke-WebRequest -UseBasicParsing http://localhost:8000/health
Invoke-WebRequest -UseBasicParsing http://localhost:3000/api-brain/health
```
...
O codigo ainda usa `supabase-py` e `@supabase/ssr` como bibliotecas cliente, mas elas devem apontar para a stack local. Nao use URLs `*.supabase.co` nos envs locais.
