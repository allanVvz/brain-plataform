...
CONTROL = COMMON | {
    ...
    "EVOLUTION_WEBHOOK_HMAC_" + "SECRET", "BULK_CAMPAIGNS_ROLLOUT1_ENABLED",
}
RUNTIME = COMMON | {
...
TRANSPORT = COMMON | {
    ...
    "EVOLUTION_API_URL", "EVOLUTION_API_KEY", "EVOLUTION_WEBHOOK_HMAC_SECRET",
...
def main() -> int:
    ...
    write_env("runtime.env", RUNTIME, source, role="brain_runtime", jwt_secret=jwt_secret,
              internal_secret=internal_secret)
    write_env("transport.env", TRANSPORT, source, role="brain_transport", jwt_secret=jwt_secret,
              internal_secret=internal_secret)
    return 0
