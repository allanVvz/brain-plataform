...
mkdir -p "$OUT_DIR"
rm -rf -- "$STAGE"
mkdir -p "$STAGE/infra" "$STAGE/api" "$STAGE/apps/conversation-runtime" \
  "$STAGE/packages" "$STAGE/supabase" "$STAGE/docs" "$STAGE/ops"
...
cp -a "$ROOT_DIR/infra/caddy-dynamic" "$ROOT_DIR/infra/microservices" "$STAGE/infra/"
cp -a "$ROOT_DIR/ops/vps" "$STAGE/ops"
cp -a "$ROOT_DIR/ops/microservices" "$STAGE/ops/"
cp -a "$ROOT_DIR/ops/release" "$STAGE/ops/"
cp -a "$ROOT_DIR/api/n8n-workflows" "$STAGE/api/"
cp -a "$ROOT_DIR/apps/conversation-runtime/n8n" "$STAGE/apps/conversation-runtime/"
cp -a "$ROOT_DIR/packages/brain-contracts" "$STAGE/packages/"
