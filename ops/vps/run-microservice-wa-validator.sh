...
if [[ "$MODE" == "--inspect" ]]; then
  ...
  session_started="$(docker exec "$runtime_name" python -c 'import sys; from services import wa_validator_service as w; print(w.get_session(sys.argv[1]).get("created_at") or "")' "$SESSION_ID")"
