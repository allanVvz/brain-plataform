...
wave through a bundle that orphans every live edge. Observed in production at
403 nodes, where the compiled branch closures collapsed to 8 nodes.
"""
...
def test_large_graph_still_returns_its_edges(monkeypatch):
    """403 nodes is the size that broke production."""
    rec = _install(monkeypatch, 403, fail_over=200)
    _nodes, edges = supabase_client.list_all_knowledge_graph(
        persona_id="p", limit_nodes=10000
    )
    assert len(edges) == 403, "edges were lost for a large graph"
...
def test_an_unreadable_graph_raises_instead_of_looking_empty(monkeypatch):
    ...
    _install(monkeypatch, 403, fail_over=0)
