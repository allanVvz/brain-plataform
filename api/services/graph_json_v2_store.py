"""Local draft/version store for Graph JSON v2.

Published documents are recorded through system events. This store is limited to
draft/apply-patch/rollback support in local-first mode.
"""
...
def _safe_slug(value: str) -> str:
    return "".join(ch if ch.isalnum() or ch in {"-", "_"} else "-" for ch in (value or "").strip().lower()).strip("-")


def _path(persona_slug: str, version: int) -> Path:
    return _DATA_ROOT / f"{_safe_slug(persona_slug)}.v{version:03d}.json"
...
def list_versions(persona_slug: str) -> list[int]:
    slug = _safe_slug(persona_slug)
    if not slug or not _DATA_ROOT.exists():
        return []
    versions: list[int] = []
    for path in _DATA_ROOT.glob(f"{slug}.v*.json"):
        try:
...
def save_version(persona_slug: str, version: int, graph: GraphJson) -> str:
    ...
    with _path(persona_slug, version).open("w", encoding="utf-8") as handle:
