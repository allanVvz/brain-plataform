"""Materialize Graph JSON v2 documents into the existing v1 graph tables."""
...
def _edge_relation(edge: Edge, source_node: dict, target_node: dict) -> str:
    relation = (edge.relation or "").strip()
    if relation and relation != "main":
        return relation
    return _default_relation(str(source_node.get("node_type") or ""), str(target_node.get("node_type") or ""))
...
def import_graph_json(*, graph_json: GraphJson, source: str = "graph_json.import", session_id: str | None = None) -> dict[str, Any]:
    ...
    for edge in graph_json.edges:
        ...
        imported = supabase_client.upsert_knowledge_edge(
            source_node["id"],
            target_node["id"],
            _edge_relation(edge, source_node, target_node),
            persona_id=persona_id,
            metadata={
                **(edge.metadata or {}),
                "primary_tree": True,
