...
## Sofia FAQ / interpretacao por galho inteiro (2026-06-02)
- Sintoma corrigido: ao clicar em gerar perguntas em um node FAQ abaixo de Copy, a tool `adaptar_faqs_universais_ao_grafo` classificava o parent imediato como `copy` e gerava perguntas internas como "Qual e a principal promessa desta copy?", expondo nome de card e ignorando o produto/ramo completo.
- Regra aplicada: quando o alvo e um FAQ e o parent imediato e `copy`, mas existe produto no caminho, a geracao passa a categoria `product` e usa o galho inteiro como contexto estrutural (`brand -> briefing/campaign -> audience -> product_group -> product -> copy -> faq`), tratando cada ligacao como "contem".
- Validacao live no backend local `127.0.0.1:8001`: para `FAQ Kit Modal 1`, `/sofia/graph-command` retornou `category=product`, objeto comercial `Kit Modal 1 (9 cores disponiveis)` e perguntas de atendimento como "Como faco para comprar..." / "e indicado para quem quer revender?", com resposta contextualizada por marca Tock Fatal, publico atacarejo, grupo Modal e produto Kit Modal 1.
- Guardrail: selecionar uma Copy diretamente ainda pode gerar perguntas sobre a copy; a mudanca vale para FAQ descendente de Copy com produto no ramo.
- Validacao automatizada: `api\.venv\Scripts\python.exe -m pytest tests\test_sofia_faq_tool.py tests\test_sofia_faq_routes.py -q` => 22 passed. Aviso residual: `.pytest_cache` sem permissao de escrita no workspace.
