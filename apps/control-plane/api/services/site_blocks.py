...
def _resolve_groups(
    ...
    for group_id in sorted(scoped):
        ...
        if group.get("node_type") not in {"product_group", "category"}:
            continue
        product_ids = sorted(i for i in _descendants(group_id, children) & scoped
                             if nodes.get(i, {}).get("node_type") == "product")
        prices = [_offer_price_cents(nodes[e["source"]]) for e in edges
                  if e.get("relation_type") == "about_product" and active(e)
                  and e["source"] in scoped and e["target"] in product_ids
                  and nodes[e["source"]].get("node_type") == "offer"]
        ...
        out.append({"node_id": group_id, "slug": group.get("slug"),
                    ...
                    **media(group_id),
                    "products": [{"node_id": i, "title": nodes[i].get("title"),
                                  "slug": nodes[i].get("slug"), **media(i)} for i in product_ids]})
    out.sort(key=lambda row: (-row["product_count"], row["node_id"]))
    return out
...
def resolve_blocks(
    ...
    *,
    ...
    edges = [edge for edge in bundle["edges"] if active(edge)]
    ...
    def in_scope(node_type: str) -> list[dict[str, Any]]:
        return [
            ...
            if node_id in nodes and ("product_group" if nodes[node_id].get("node_type") == "category" else nodes[node_id].get("node_type")) == node_type
        ]
