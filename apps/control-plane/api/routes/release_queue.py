...
def list_queue(
    ...
    status: str | None = Query(None, max_length=40), persona_id: str | None = Query(None),
    offset: int = Query(0, ge=0),
    limit: int = Query(50, ge=1, le=100),
):
    user = auth_service.current_user(request)
    if persona_id:
        # The dashboard sends the selected persona explicitly.  Enforce the
        # scope before paging so an admin-wide queue cannot hide that persona's
        # rows behind another persona's first page.
        auth_service.assert_persona_capability(request, "view", persona_id=persona_id)
        scope = [persona_id]
