...
def _create_reactivation_preview_line(
    *, persona_slug: str, lead_ref: int, source_buffer_id: str,
    actor_user_id: str | None = None, line_kind: str = "apology",
    sequence_index: int = 1,
    reactivation_group_id: str | None = None,
    preview_revision: int = 1,
    identity_override: str | None = None,
    publication_override: dict[str, Any] | None = None,
    contextual_preview: dict[str, Any] | None = None,
    enqueue: bool = True,
) -> dict[str, Any]:
    """Turn published or agent-generated proactive content into a reviewable envelope.

    The apology line comes from published graph copy. The contextual line is
    generated by the same two-stage runtime used for conversation turns and
    arrives here only after its proof has preserved the model reply. Transport
    performs the final source/new-inbound/consent checks atomically.
    """
    persona = supabase_client.get_persona(persona_slug) or {}
    lead = supabase_client.get_lead_by_ref(lead_ref) or {}
    ...
    publication = publication_override or supabase_client.get_active_graph_publication(str(persona["id"])) or {}
    document = publication.get("document_json") or {}
    ...
    if not rule_node_id or (rule_node or {}).get("node_type") != "rule":
        raise RuntimeError("published reactivation rule is unavailable")
    if line_kind == "context":
        if not contextual_preview:
            raise RuntimeError("contextual reactivation preview is unavailable")
        if str(contextual_preview.get("publication_id") or "") != str(publication.get("id") or ""):
            raise RuntimeError("contextual reactivation preview publication changed")
        if str(contextual_preview.get("graph_checksum") or "") != str(publication.get("checksum") or ""):
            raise RuntimeError("contextual reactivation preview graph changed")
        text = str(contextual_preview.get("text") or "").strip()
        proof_result = dict(contextual_preview.get("proof_result") or {})
        if not text or proof_result.get("valid") is not True or proof_result.get("delivery_authorized") is not True:
            raise RuntimeError("contextual reactivation preview proof failed")
        copy_node_id = None
        evidence_node_ids = list(dict.fromkeys([rule_node_id, *(contextual_preview.get("evidence_node_ids") or [])]))
        model_proposal = dict(contextual_preview.get("model_proposal") or {})
    else:
        copy_keys = ("operator_reactivation_copy_node_id", "morning_reactivation_copy_node_id")
        copy_node_id = str(next((hours.get(key) for key in copy_keys if hours.get(key)), "")).strip()
        copy_node = next(
        ...
        if (
            ...
            raise RuntimeError("published reactivation copy is unavailable")
    ...
    group_id = reactivation_group_id or f"proactive-reactivation:{source_buffer_id}"
    identity = identity_override or f"{group_id}:{line_kind}:{sequence_index}:r{preview_revision}"
    request = {
        "source_buffer_id": source_buffer_id,
        "lead": lead,
        "text": text,
        ...
        "metadata": ({
            "published_business_hours": {
                "timezone": hours.get("timezone"), "start": hours.get("start"), "end": hours.get("end"),
                "graph_version": publication.get("version"), "graph_checksum": publication.get("checksum"),
                "publication_id": publication.get("id"),
            }
        } if enqueue else None),
        "queue_group_id": group_id,
        "queue_parent_buffer_id": source_buffer_id,
        "queue_line_kind": line_kind,
        "queue_sequence": sequence_index,
        "queue_revision": preview_revision,
    }
    if enqueue:
        request["actor_user_id"] = actor_user_id
        return transport_client.enqueue_reactivation_preview(**request)
    return request
...
def create_reactivation_preview(
    *, persona_slug: str, lead_ref: int, source_buffer_id: str,
    ...
    """Compatibility entry point for one published apology line."""
    return _create_reactivation_preview_line(
        persona_slug=persona_slug, lead_ref=lead_ref, source_buffer_id=source_buffer_id,
        actor_user_id=actor_user_id, identity_override=f"proactive-reactivation:{source_buffer_id}",
    )
...
def create_reactivation_pair(
    *, persona_slug: str, lead_ref: int, source_buffer_id: str,
    ...
    recent = supabase_client.get_messages(str(lead.get("id") or lead_ref), limit=12)
    inbound = [row for row in recent if str(row.get("direction") or row.get("role") or "").lower() in {"inbound", "user", "customer"}]
    latest_context = str((inbound[-1] if inbound else {}).get("content") or (inbound[-1] if inbound else {}).get("text") or "").strip()
    ...
    # pair before any transport action is allowed.
    from services import agentic_turn
    contextual_result = agentic_turn.execute(
        persona_slug=persona_slug,
        lead_ref=lead_ref,
        message=latest_context,
        message_id=f"reactivation-context:{source_buffer_id}",
        correlation_id=f"reactivation-context:{source_buffer_id}",
        phone_number_id=str(lead.get("whatsapp_phone_number_id") or "") or None,
        channel_binding_id=str(lead.get("channel_binding_id") or ""),
        inbound_buffer_id=str(source_buffer_id),
        publication_id=str(publication["id"]),
        preview_only=True,
        commit_result=False,
    )
    contextual_context = contextual_result.get("context")
    contextual_response = contextual_result.get("response")
    contextual_decision = contextual_result.get("decision")
    if contextual_context is None or contextual_response is None or contextual_decision is None:
        raise RuntimeError("contextual reactivation did not produce a proved response")
    contextual_proof = {
        **dict(contextual_response.proof or {}),
        "kind": "operator_requested_contextual_reactivation",
        "publication_id": contextual_context.publication_id,
        "graph_checksum": contextual_context.graph_checksum,
        "evidence_node_ids": list(contextual_response.evidence_node_ids or []),
        "handoff_required": bool(contextual_response.handoff_required),
        "handoff_reason": contextual_decision.handoff_reason,
    }
    contextual_preview = {
        "text": contextual_response.reply_text,
        "publication_id": contextual_context.publication_id,
        "graph_checksum": contextual_context.graph_checksum,
        "evidence_node_ids": list(contextual_response.evidence_node_ids or []),
        "proof_result": contextual_proof,
        "model_proposal": (
            contextual_response.proposal.model_dump(mode="json")
    ...
    second = _create_reactivation_preview_line(
        persona_slug=persona_slug, lead_ref=lead_ref, source_buffer_id=source_buffer_id,
        actor_user_id=actor_user_id, line_kind="context", sequence_index=2,
        reactivation_group_id=group_id, preview_revision=revision,
        publication_override=publication, contextual_preview=contextual_preview, enqueue=False,
    )
    result = transport_client.enqueue_reactivation_pair(
        source_buffer_id=source_buffer_id, lead=lead,
