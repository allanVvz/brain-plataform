...
- `/internal/v1/conversations/resolve-understanding` validates the first,
  language-free model result and computes prospective state plus scoped RAG;
- `/internal/v1/conversations/decide` accepts either the compatible single-pass
  `model_observation` or a resolved understanding plus `ConversationReplyV1`;
...
The published graph selects execution per role. `interpret_then_respond` runs
`TurnUnderstandingV1 -> read-only resolution/RAG -> ConversationReplyV1 ->
proof -> one commit`; `single_pass` remains compatible. A failed model call,
