...
export default function ValidatePage() {
  ...
  useEffect(() => {
    load();
  }, [filterStatus, filterType, filterPersona]);

  async function approve(id: string) {
    setProcessing(id);
    try {
      await api.approveItem(id, false);
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-semibold">Validacao de Conhecimento</h1>
          <p className="text-sm text-brain-muted mt-0.5">Revise, aprove ou rejeite materiais sincronizados</p>
        </div>
        <button
          onClick={load}
          className="text-xs text-brain-muted hover:text-white border border-brain-border px-3 py-1.5 rounded-md"
        >
          Atualizar
        </button>
      </div>
      ...
      <div className="flex gap-2 flex-wrap">
        {tabs.map(({ key, label, urgent }) => {
          ...
          return (
            ...
            >
              {urgent && count > 0 && <span className="w-1.5 h-1.5 rounded-full bg-orange-400 shrink-0" />}
              {label}
              {count > 0 && (
                <span
      ...
      {!loading && items.length === 0 && <p className="text-brain-muted text-sm">Nenhum item neste filtro.</p>}
        ...
        {items.map((item) => (
          <div
            ...
            className={`border rounded-xl overflow-hidden ${
              expanded === item.id ? "border-brain-accent/60" : "border-brain-border"
            }`}
          >
            <div
            ...
            >
              <span
                className={`text-xs px-2 py-0.5 rounded border font-medium shrink-0 ${
                  STATUS_COLORS[item.status] ?? "text-brain-muted border-brain-border"
                }`}
              >
                {item.status}
              </span>
              ...
              <div className="border-t border-brain-border bg-brain-bg/50 p-4 space-y-4">
                ...
                {item.content_type === "asset" && (
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    <div>
                      <label className="text-xs text-brain-muted block mb-1">Funcao do asset</label>
                      <select
                ...
                <div className="flex gap-2 pt-1 flex-wrap">
                  ...
                  {item.status === "approved" && item.content_type === "faq" && (
                ...
                {(item.status === "needs_persona" || !item.persona_id) && (
                  <p className="text-xs text-orange-400 bg-orange-500/10 border border-orange-500/20 rounded-lg px-3 py-2">
                    Atribua uma persona para liberar a aprovacao.
                  </p>
                )}
                {item.status === "needs_category" && (
                  <p className="text-xs text-amber-400 bg-amber-500/10 border border-amber-500/20 rounded-lg px-3 py-2">
                    Selecione um tipo de conteudo diferente de "other" para liberar a aprovacao.
                  </p>
                )}
              </div>
            )}
