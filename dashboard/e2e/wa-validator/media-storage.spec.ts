...
test("armazena e reabre PNG/PDF no WA Validator sem outbound real", async ({ browser }, testInfo: TestInfo) => {
  ...
  const assets = mediaResponses.map(({ status, body }, index) => {
    ...
    expect(body.graph_attachment?.attached).toBe(true);
    expect(body.graph_attachment?.conversation_node_id).toBeTruthy();
    expect(body.graph_attachment?.asset_node_id).toBeTruthy();
    expect(body.graph_attachment?.conversation_asset_edge_id).toBeTruthy();
    expect(body.graph_attachment?.gallery_node_id).toBeTruthy();
    expect(body.graph_attachment?.asset_gallery_edge_id).toBeTruthy();
    return body.asset;
  });
