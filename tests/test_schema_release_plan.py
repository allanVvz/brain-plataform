...
def test_schema_plan_ends_at_manifest_version_and_is_checksummed(tmp_path):
    ...
    assert plan["schema_version"] == 135
    assert plan["target_migration"] == "135_microservice_storage_role_grants.sql"
    assert plan["migrations"][-1]["version"] == 135
