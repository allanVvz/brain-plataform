...
def test_schema_plan_ends_at_manifest_version_and_is_checksummed(tmp_path):
    ...
    assert plan["schema_version"] == 154
    assert plan["target_migration"] == "154_persist_queue_preview_and_preserve_position.sql"
    assert plan["migrations"][-1]["version"] == 154
...
def test_current_pending_migrations_are_atomic_compatible():
    ATOMIC.validate_filenames(
        [
            ...
            "154_persist_queue_preview_and_preserve_position.sql",
        ]
    )
