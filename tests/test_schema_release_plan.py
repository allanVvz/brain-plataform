...
def test_schema_plan_ends_at_manifest_version_and_is_checksummed(tmp_path):
    ...
    assert plan["schema_version"] == 134
    assert plan["target_migration"] == "134_business_hours_release_queue.sql"
    assert plan["migrations"][-1]["version"] == 134
