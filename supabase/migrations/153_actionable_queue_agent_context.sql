-- Make the operator-facing queue answer the operational question: what did
-- the agent say before this item? Keep the latest customer message and the
-- last three combined messages as explicit context instead of overloading
-- the legacy latest_message field.
CREATE OR REPLACE FUNCTION public.list_actionable_message_queue_v149(
  ...
  WITH scoped AS (
    ...
    LEFT JOIN LATERAL (
      SELECT coalesce(jsonb_agg(jsonb_build_object(
        'id',recent.id,
        'content',left(coalesce(recent.content,''),2000),
    ...
    LEFT JOIN LATERAL (
      ...
      ORDER BY first_line.created_at DESC
      LIMIT 1
  ...
  SELECT jsonb_build_object(
    'items',coalesce((SELECT jsonb_agg(jsonb_build_object(
      ...
      'status',queue_state,'queue_state',queue_state,
      ...
      'latest_inbound_context',latest_inbound_context,
      'recent_context',coalesce(recent_context,'[]'::jsonb),
