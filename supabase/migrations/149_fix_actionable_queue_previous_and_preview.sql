-- Keep sent outbounds out of the "previous message" column and out of the
-- preview column. The dashboard must show the last customer message, while a
-- preview is only a not-yet-sent generated line.
CREATE OR REPLACE FUNCTION public.list_actionable_message_queue_v1(
  ...
  SELECT jsonb_build_object(
    'items',coalesce((SELECT jsonb_agg(jsonb_build_object(
      ...
      'previous_message',coalesce(CASE WHEN queue_sequence=2 THEN first_preview_text ELSE NULL END,latest_message),
