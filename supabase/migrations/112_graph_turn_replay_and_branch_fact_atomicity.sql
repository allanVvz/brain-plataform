-- Make branch switches and their branch-owned facts one atomic state change.
-- No storage is added: immutable fact history and the existing turn proof are
-- the idempotency/audit ledger.
...
DECLARE
  ...
  v_switched_previous uuid;
  v_next_revision bigint;
  v_invalidated_keys text[];
  v_previous_branch text;
  v_branch_action text := coalesce(p_model_proposal->>'branch_action', 'keep');
BEGIN
  ...
  -- Replay is authoritative and must be resolved before comparing a stale
  -- caller revision. A provider/n8n retry returns the original proof/outbox.
  ...
  IF NOT FOUND THEN
    IF coalesce(p_expected_revision, 0) <> 0 THEN
      RAISE EXCEPTION 'conversation_ledger_revision_cas: expected %, current absent', p_expected_revision
        USING ERRCODE = 'P0001',
              DETAIL = 'conversation_ledger_revision_cas';
    END IF;
  ...
  FOR v_fact IN SELECT value FROM jsonb_array_elements(coalesce(p_facts, '[]'::jsonb)) LOOP
    ...
    -- A graph-authorized switch replaces branch-owned facts from the branch
    -- being left. Persona-owned facts remain current because their owner is
    -- not the previous branch. "add" deliberately preserves both owners.
    IF v_branch_action = 'switch'
       AND v_previous_branch IS NOT NULL
       AND v_previous_branch IS DISTINCT FROM p_active_branch_node_id
       AND v_fact->>'owner_node_id' = p_active_branch_node_id THEN
      SELECT id INTO v_switched_previous
        FROM public.conversation_facts
       WHERE ledger_id = v_ledger.id AND field_key = v_fact->>'field_key'
